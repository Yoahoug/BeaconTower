...
自托管服务器监控面板：企业级 dense 风格前端，通过 SSH 直连被监控服务器采集指标（无 Agent），Docker 单容器部署，公开页面不暴露 IP 等敏感信息。支持 RAPL 功耗监控（实时功率 / kWh / 电费）。

## 当前状态：前端企业级 v1.0（含完整管理面板 + 功耗展示）
...
- `/` 监控状态总览（KPI 条 + 全网吞吐/功耗趋势 + 节点网格 + 搜索/筛选，10 秒静默刷新）
...
- 前端开发约束见 [doc/10-前端开发约束.md](./doc/10-前端开发约束.md)（合并门槛：a11y 自查 + 性能预算）
