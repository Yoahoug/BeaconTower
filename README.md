...
自托管服务器监控面板：亮色卡片式前端，通过 SSH 直连被监控服务器采集指标（无 Agent），Docker 单容器部署，公开页面不暴露 IP 等敏感信息。

## 当前状态：前端原型

完整开发方案见 [doc/README.md](./doc/README.md)。前端原型（液态玻璃风格卡片墙 + 环形仪表，模拟数据）已可运行：
...
- `/` 监控状态总览（卡片墙 + 汇总统计，10 秒静默刷新）
- `/admin` 管理面板风格示意（占位页）

> 所有数据均为前端模拟数据，不含真实服务器信息。
